window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // ---- QR code encoder (byte mode, EC M, auto version) ----
    ...
    function appendBits(arr, val, len) {
      for (var i = len - 1; i >= 0; i--) arr.push(((val >>> i) & 1) !== 0);
    }
    ...
    function buildQrMatrix(text, ecl) {
      ...
      var usedBits = totalBitsByte(bytes.length, version);
      var bb = [];
      appendBits(bb, 0x4, 4);
      appendBits(bb, bytes.length, version <= 9 ? 8 : 16);
      for (var b of bytes) appendBits(bb, b, 8);
      ...
      for (var i = 0; i < bb.length; i += 8) {
        var v = 0;
        for (var j = 0; j < 8; j++) v = (v << 1) | (bb[i + j] ? 1 : 0);
        data.push(v);
      }
      ...
      for (var i = 0; i < align.length; i++) {
        for (var j = 0; j < align.length; j++) {
          ...
          var ax = align[i], ay = align[j];
          for (var dy = -2; dy <= 2; dy++) {
            for (var dx = -2; dx <= 2; dx++) setFunction(ax + dx, ay + dy, Math.max(Math.abs(dx), Math.abs(dy)) !== 1);
          }
        }
      }
      const FORMAT_MASK = 0x5412;
      function formatBitsValue(mask) {
        var d = (ECC_FORMAT_BITS[ecl] << 3) | mask;
        var rem = d;
        for (var i = 0; i < 10; i++) rem = (rem << 1) ^ ((rem >>> 9) * 0x537);
        return ((d << 10) | rem) ^ FORMAT_MASK;
      }
      function setFormatBits(mask) {
        var bits = formatBitsValue(mask);
        for (var i = 0; i <= 5; i++) setFunction(8, i, getBit(bits, i));
        setFunction(8, 7, getBit(bits, 6));
        setFunction(8, 8, getBit(bits, 7));
        setFunction(7, 8, getBit(bits, 8));
        for (var i = 9; i < 15; i++) setFunction(14 - i, 8, getBit(bits, i));
        for (var i = 0; i < 8; i++) setFunction(size - 1 - i, 8, getBit(bits, i));
        for (var i = 8; i < 15; i++) setFunction(8, size - 15 + i, getBit(bits, i));
      ...
      function penalty(mat) {
        ...
        var dark = 0;
        for (var y = 0; y < size; y++) for (var x = 0; x < size; x++) if (mat[y][x]) dark++;
        var total = size * size;
      ...
      function applyMask(mat, mask) {
        ...
        var bits = formatBitsValue(mask);
        var setF = function (x, y, v) { m[y][x] = v; };
    ...
    function qrIcon() {
      var finders = [[0, 0], [9, 0], [0, 9]];
      var d = "";
      for (var p of finders) {
    ...
    // ---- host API helpers (same-origin web-server routes) ----
    function apiInfo() { return fetch("/__qr/info").then(function (r) { return r.json(); }); }
    function apiRotate() { return fetch("/__qr/rotate", { method: "POST" }).then(function (r) { return r.json(); }); }
    function apiGetConfig() { return fetch("/__qr/config").then(function (r) { return r.json(); }); }
    function apiSetConfig(c) {
      return fetch("/__qr/config", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(c) }).then(function (r) { return r.json(); });
    }
    ...
    function ConfigCard(props) {
      ...
      var stateRef = React.useState({ status: "loading", port: "", days: "", refresh: "" });
      var state = stateRef[0], setState = stateRef[1];
      var msgRef = React.useState("");
      var msg = msgRef[0], setMsg = msgRef[1];
      var dirtyRef = React.useState(false);
      var dirty = dirtyRef[0], setDirty = dirtyRef[1];
      React.useEffect(function () {
        apiGetConfig().then(function (c) {
          setState({ status: "ready", port: String(c && c.port), days: String(c && c.sessionDays), refresh: String(c && c.refreshSeconds) });
        }).catch(function () { setState({ status: "error", port: "", days: "", refresh: "" }); });
      }, []);
      function reload() {
        apiGetConfig().then(function (c) {
          setState({ status: "ready", port: String(c && c.port), days: String(c && c.sessionDays), refresh: String(c && c.refreshSeconds) });
      ...
      function save() {
        ...
        var days = Number(state.days);
        var refresh = Number(state.refresh);
        ...
        apiSetConfig({ port: port, sessionDays: days, refreshSeconds: refresh }).then(function (c) {
          setState({ status: "ready", port: String(c && c.port), days: String(c && c.sessionDays), refresh: String(c && c.refreshSeconds) });
          setDirty(false);
          setMsg("");
        }).catch(function () { setMsg(t("card.saveFailed")); });
      }
      return React.createElement("li", { className: "dshqr-card" + (open ? " dshqr-card-open" : "") },
        React.createElement("button", { type: "button", className: "dshqr-card-header", "aria-expanded": open, onClick: function () { setOpen(!open); } },
        ...
        open ? React.createElement("div", { className: "dshqr-card-body" },
          React.createElement("div", { className: "dshqr-card-row" },
            ...
            React.createElement("input", { className: "dshqr-card-input", type: "number", min: 1, max: 65535, value: state.port, onChange: function (e) { setState(Object.assign({}, state, { port: e.target.value })); setDirty(true); } })
          ),
          React.createElement("div", { className: "dshqr-card-row" },
            React.createElement("label", { className: "dshqr-card-label" }, t("card.sessionDays")),
            React.createElement("input", { className: "dshqr-card-input", type: "number", min: 1, max: 3650, value: state.days, onChange: function (e) { setState(Object.assign({}, state, { days: e.target.value })); setDirty(true); } })
          ),
          React.createElement("div", { className: "dshqr-card-row" },
            React.createElement("label", { className: "dshqr-card-label" }, t("card.refresh")),
            React.createElement("input", { className: "dshqr-card-input", type: "number", min: 0, max: 86400, value: state.refresh, onChange: function (e) { setState(Object.assign({}, state, { refresh: e.target.value })); setDirty(true); } })
          ),
    ...
    function QrConnect(props) {
      ...
      var closing = closingState[0], setClosing = closingState[1];
      var stateRef = React.useState({ status: "idle", localUrl: null, publicUrl: null });
      var state = stateRef[0], setState = stateRef[1];
      var leftRef = React.useState(30);
      var left = leftRef[0], setLeft = leftRef[1];
      var daysRef = React.useState("30");
      var days = daysRef[0], setDays = daysRef[1];
      var msgRef = React.useState("");
      var msg = msgRef[0], setMsg = msgRef[1];
      var copiedRef = React.useState("");
      var copied = copiedRef[0], setCopied = copiedRef[1];
      var refreshRef = React.useState(30);
      var refreshSecs = refreshRef[0], setRefreshSecs = refreshRef[1];

      function buildUrls(info) {
        if (!info || !info.secret || !info.port) return { localUrl: null, publicUrl: null };
        var make = function (host) { return "http://" + host + ":" + info.port + "/?auth=" + info.secret; };
        return {
      ...
      function applyInfo(info) {
        ...
        var rs = (info && typeof info.refreshSeconds === "number") ? info.refreshSeconds : 30;
        setRefreshSecs(rs);
      }
      function refresh() {
        return apiInfo().then(applyInfo).catch(function () {
          setState({ status: "error", localUrl: null, publicUrl: null });
        });
      }
      function manualRefresh() {
        return apiRotate().then(applyInfo).catch(function () {
          setState({ status: "error", localUrl: null, publicUrl: null });
        });
      }
      React.useEffect(function () {
      ...
      function applyDays() {
        ...
        apiSetConfig({ sessionDays: n }).then(function (r) {
          setMsg(t("panel.daysApplied", { n: (r && r.sessionDays) }));
        }).catch(function () { setMsg(t("panel.failedApply")); });
      }
      function copyUrl(url) {
        if (!url) { setCopied(t("panel.noLink")); return; }
        try {
          if (navigator && navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(url).then(function () { setCopied(t("panel.copied")); }, function () { setCopied(t("panel.copyFailed")); });
          } else {
      ...
      var localBlock = state.status === "ready" ? qrBlock(t("panel.local"), state.localUrl, false) : null;
      var publicBlock = state.status === "ready" ? qrBlock(t("panel.public"), state.publicUrl, true) : null;
          ...
          React.createElement("div", { className: "dshqr-body" },
            ...
            React.createElement(React.Fragment, null,
              ...
              React.createElement("div", { className: "dshqr-config" },
                ...
                React.createElement("input", { className: "dshqr-input", type: "number", min: 1, max: 3650, value: days, onChange: function (e) { setDays(e.target.value); } }),
    ...
    function apply(ctx) {
      ctx.effect(function () { return ctx.locale.register(NS, { zh: ZH, en: EN }); }, "dsh-plugin-qr-connect: locale");

      ctx.slots.inject("settings.plugin.item", function () {
        return ctx.slots.register(
          { name: "settings.plugin.item", key: "qr-connect", locale: NS },
          ConfigCard
        );
      });
