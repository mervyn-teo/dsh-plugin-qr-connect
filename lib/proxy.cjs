...
const stateFile = arg('--state-file', path.join(os.tmpdir(), 'dsh-lan-proxy-state.json'))
...
// Portable manual rotation: the host writes "rotate\n" to stdin, which works on
// Windows (SIGUSR1 is unavailable there).
process.stdin.setEncoding('utf8')
process.stdin.resume()
process.stdin.on('data', function (chunk) {
  if (String(chunk).indexOf('rotate') !== -1) rotate()
})
